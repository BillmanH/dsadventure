...
# Character origin tweaks the character when created to have specific advantages
def set_char_origin(c, world):
    ...
    if c.background == 'Noble family':
        ...
        world.df_features.loc[world.df_features['nation']
                              == t.nation, 'aware'] = 1
...
def set_skills(world, character_dict):
    ...
    if skill == 'mountaneer':
        world.df_features.loc[world.df_features['terrain']
                              == 'mountain', 'aware'] = 1
    ...
    if skill == 'additional language':
        world.Character.languages.append(np.random.choice(
            [n.name for n in world.nations if n.name != world.Character.birthNation]))
...
def create_character(character_dict, world):
    ...
    world.Character.languages.append(
        world.df_features.loc[world.Character.get_location_key()].nation)
    return world
