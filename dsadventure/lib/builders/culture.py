...
class Names:
    def __init__(self):
        try:
            ...
            self.nameparts = pd.read_csv("dsadventure/lib/Datasets/namegen.csv")
            self.townnameparts = pd.read_csv(
                "dsadventure/lib/Datasets/townnameparts.csv"
            )
...
class Culture:
    ...
    def __init__(self, params=default_params, paths=None):
        ...
        self.town_birthrate = params.get("town_birthrate", 0.3)
        ...
        # divine forces
        self.chaos = params.get("chaos", 0.6)
    ...
    def townNameGenerator(self):
        ...
        return (
            (
                np.random.choice(self.names.prefixes)
    ...
    def personNameGenerator(self):
        return (
            str(np.random.choice(self.names.nameparts["1"]))
