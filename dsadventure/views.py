...
def core_view(request):
    ...
    if "POST" == request.method:
        ...
        world.df_features.loc[context["old_location"], ["visited", "aware"]] = 1
        world.df_features.loc[
            context["old_location"], "turn_last_visited"
    ...
    context["n_ter_items"] = [
        {"texture": tt[i], "detail": [t for t in td if t["name"] == tt[i]["name"]][0]}
...
@login_required
def journal(request):
    context = {"relationships": {}, "diplomacy": {}}
    world = b.get_world(request.user.get_username())
...
def create_character(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            character_dict = {
                "name": f["name"],
                "background": dict(playerCharacterForm.backgroundChoices)[
                    f["background"]
                ],
                "coreskills": dict(playerCharacterForm.coreSkillsChoices)[
                    f["coreskills"]
                ],
                "secondaryskills": dict(playerCharacterForm.secondarySkillsChoices)[
                    f["secondaryskills"]
                ],
            }
...
def generate_world(request):
    ...
    if "GET" == request.method:
        ...
        wa = [
            world.df_features.loc[m].fillna("").to_dict()
        ...
        if "phase" not in context["formData"].keys():
            context["formData"]["phase"] = 1
        context["formData"] = yaml.load(
            request.POST.get("formData", "No data found"), yaml.SafeLoader
        )
